...
<Tabs syncKey="package-manager">
	...
	```bash
	npm i @syntatis/kubrick
	```
	</TabItem>
	<TabItem label="yarn" icon="seti:yarn">
	```bash
	yarn add @syntatis/kubrick
	```
	</TabItem>
	<TabItem label="pnpm" icon="pnpm">
	```bash
	pnpm add @syntatis/kubrick
	```
	</TabItem>
</Tabs>

Keep in mind that `react` and `react-dom` is set as peer dependencies, requires version 18 or the later. So make sure that you have them installed and set them as the depenendencies in your project.

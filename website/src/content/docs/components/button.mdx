...
<Lead
	...
	stackblitz="/edit/syntatis-kubrick-button?file=src/App.jsx">
...
<Table
	...
	items={[
		[
			"onPress",
			"Triggered when the button is clicked. It will receive the click event as an argument.",
		],
		[
			"onHoverChange",
			"Triggered when the button is hovered. It will receive a boolean indicating if the button is hovered.",
		],
		[
			"onFocusChange",
			"Triggered when the button receives or loses focus. It will receive a boolean indicating if the button is focused.",
		],
	]}
/>

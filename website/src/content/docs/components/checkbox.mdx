...
<Lead
	...
	stackblitz="/edit/syntatis-kubrick-checkbox?file=src/App.jsx">
...
<Table
	...
	items={[
		[
			"onChange",
			"Triggered when the check state changes. It will receive the new check state as a boolean.",
		],
		[
			"onFocusChange",
			"Triggered when the input receives or loses focus. It will receive a boolean indicating if the input is focused.",
		],
	]}
/>

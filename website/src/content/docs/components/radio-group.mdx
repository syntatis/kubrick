...
The `RadioGroup` component helps manage a group of `Radio` buttons. It handles the state for the group and provides the necessary accessibility features. This component is useful when you need to show multiple options, but the user should only pick one.

## Usage

To use the `RadioGroup` component in your WordPress admin application, you will need to import both the `RadioGroup` and `Radio` components from the package:

```jsx
import { RadioGroup, Radio } from '@syntatis/kubrick';
```

In your application, you can place multiple `Radio` components inside the `RadioGroup`. The `RadioGroup` should have the `label` prop to describe the group, and each `Radio` should have a unique `value` to identify the selected option.

```jsx
<RadioGroup label="Choose your favorite color">
...
<Aside type="tip">
Changing the layout to horizontal is useful when you have a small number of options. If you have a long list of options, it's better to keep the default vertical layout.
</Aside>

## Events

The `RadioGroup` component emits an event when users select or change the option. To handle this event, you can pass a function as a callback on the following props:
...
The `RadioGroup` component comes with some static selectors, prefixed with `.kubrick-RadioGroup-`. You can use these classes to select and customize the component and its elements.

<div className="table-classes">
| Selector | Description |
| --- | --- |
| `root` | The root element of `RadioGroup` component, a `div` |

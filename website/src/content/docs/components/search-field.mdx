...
<Lead
	storybook="/docs/components-searchfield--docs"
	stackblitz="/edit/syntatis-kubrick-search-field?file=src/App.jsx">
...
<Table
	...
	component="SearchField"
	items={[
		["onChange", "Triggered when the input value changes. The event handler receives the new input value as an argument."],
		["onSubmit", "Triggered when the user submits the search query. The event handler receives the search query as an argument."],
		["onClear", "Triggered when the user clears the search field. The event handler receives the search query as an argument."],
	]}
	/>
